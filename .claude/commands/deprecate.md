...
Deprecation and migration workflows: safe removal, backward compat periods, communication plans.

Read the full skill workflow and follow it step by step:

```
cat ~/.agent-toolkit/vendor/agent-skills/skills/deprecation-and-migration/SKILL.md
```

If the skill references additional files in its directory, read those too:

```
ls ~/.agent-toolkit/vendor/agent-skills/skills/deprecation-and-migration/
```
...
curl -sf -X POST http://localhost:3111/agentmemory/smart-search \
  ...
  -d '{"query": "deprecate deprecation-and-migration"}' 2>/dev/null || echo "Memory not available"
```
