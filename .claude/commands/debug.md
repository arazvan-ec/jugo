...
Systematic debugging: stop-the-line, 5-step triage (reproduce → localize → reduce → fix → guard).

Read the full skill workflow and follow it step by step:

```
cat ~/.agent-toolkit/vendor/agent-skills/skills/debugging-and-error-recovery/SKILL.md
```

If the skill references additional files in its directory, read those too:

```
ls ~/.agent-toolkit/vendor/agent-skills/skills/debugging-and-error-recovery/
```
...
curl -sf -X POST http://localhost:3111/agentmemory/smart-search \
  ...
  -d '{"query": "debug debugging-and-error-recovery"}' 2>/dev/null || echo "Memory not available"
```
