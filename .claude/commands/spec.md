...
**Phase: Define** — Run the spec-driven development workflow. Write structured specifications before code: vision → PRD → tasks → implement.
...
```bash
ls ~/.agent-toolkit/vendor/agent-skills/skills/spec-driven-development/
```
...
curl -sf -X POST http://localhost:3111/agentmemory/smart-search \
  ...
  -d '{"query": "spec spec-driven-development"}' 2>/dev/null || echo "Memory not available"
```
