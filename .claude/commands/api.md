...
API design: REST/GraphQL patterns, error semantics, versioning, boundary design, input validation, backward compat.

Read the full skill workflow and follow it step by step:

```
cat ~/.agent-toolkit/vendor/agent-skills/skills/api-and-interface-design/SKILL.md
```

If the skill references additional files in its directory, read those too:

```
ls ~/.agent-toolkit/vendor/agent-skills/skills/api-and-interface-design/
```
...
curl -sf -X POST http://localhost:3111/agentmemory/smart-search \
  ...
  -d '{"query": "api api-and-interface-design"}' 2>/dev/null || echo "Memory not available"
```
