...
CI/CD automation: pipeline design, test/lint/typecheck enforcement, deployment strategies, failure feedback loops.

Read the full skill workflow and follow it step by step:

```
cat ~/.agent-toolkit/vendor/agent-skills/skills/ci-cd-and-automation/SKILL.md
```

If the skill references additional files in its directory, read those too:

```
ls ~/.agent-toolkit/vendor/agent-skills/skills/ci-cd-and-automation/
```
...
curl -sf -X POST http://localhost:3111/agentmemory/smart-search \
  ...
  -d '{"query": "cicd ci-cd-and-automation"}' 2>/dev/null || echo "Memory not available"
```
