...
Production UI: component architecture, design systems, state management, responsive design, WCAG 2.1 AA accessibility.

Read the full skill workflow and follow it step by step:

```
cat ~/.agent-toolkit/vendor/agent-skills/skills/frontend-ui-engineering/SKILL.md
```

If the skill references additional files in its directory, read those too:

```
ls ~/.agent-toolkit/vendor/agent-skills/skills/frontend-ui-engineering/
```
...
curl -sf -X POST http://localhost:3111/agentmemory/smart-search \
  ...
  -d '{"query": "frontend frontend-ui-engineering"}' 2>/dev/null || echo "Memory not available"
```
